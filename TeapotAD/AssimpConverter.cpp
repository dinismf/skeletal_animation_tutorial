	...
	static void AddMeshesAndBones(const aiScene* a_pScene, SkeletalModel& a_OutModel)
	{
		...
		for (unsigned int i = 0; i < a_pScene->mNumMeshes; ++i)
		{
			...
			for (GLuint i = 0; i < pMesh->mNumVertices; ++i) {
				...
				tempPosition.x = pMesh->mVertices[i].x;
				tempPosition.y = pMesh->mVertices[i].y;
				tempPosition.z = pMesh->mVertices[i].z;
				vertex.position = tempPosition;

				tempNormal.x = pMesh->mNormals[i].x;
				tempNormal.y = pMesh->mNormals[i].y;
				tempNormal.z = pMesh->mNormals[i].z;
				...
				AnimMesh.aVertexData.push_back(vertex);
			}
			...
			for (unsigned int i = 0; i < pMesh->mNumFaces; ++i)
			{
				...
				for (GLuint j = 0; j < Face.mNumIndices; j++) {
					...
					AnimMesh.vIndices.push_back(Face.mIndices[j]);
				}
			}
			//
